...
async function getUserMyCrops(userEmail) {
  ...
  // 1. Check local farmer profile first (from Profile Settings)
  try {
    const profStr = localStorage.getItem('agriguard_farmer_profile');
    if (profStr) {
      const prof = JSON.parse(profStr);
      if (prof.primary_crops && Array.isArray(prof.primary_crops) && prof.primary_crops.length > 0) {
        crops = prof.primary_crops.map(c => typeof c === 'string' ? c : (c.name || c.id));
      }
    }
  } catch {}

  // 2. Query Supabase Cloud FarmerProfile for logged-in user
  if (!crops || crops.length === 0) {
    try {
      if (userEmail) {
        const res = await fetch(`${SUPA_URL}/rest/v1/FarmerProfile?uid=eq.${encodeURIComponent(userEmail)}`, { headers: supaHeaders() });
        if (res.ok) {
          const rows = await res.json();
          if (rows[0]?.primary_crops?.length) {
            crops = rows[0].primary_crops.map(c => typeof c === 'string' ? c : (c.name || c.id));
          }
        }
      }
    } catch {}
  ...
  // 3. Fallback to agriguard_my_crops
  if (!crops || crops.length === 0) {
    try {
      ...
      if (local) {
        ...
        if (Array.isArray(arr) && arr.length > 0) {
          crops = arr.map(c => typeof c === 'string' ? c : (c.name || c.id));
        }
      }
    } catch {}
  }
...
export default function WeatherScreen() {
  ...
  return (
    <div className="min-h-screen bg-[#052e16]">
      ...
      {/* Tab bar */}
      <div className="bg-[#052e16] px-4 sticky top-0 z-10">
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-all ${
                activeTab === tab
                  ? "bg-[#4ade80] text-[#052e16] shadow-md"
